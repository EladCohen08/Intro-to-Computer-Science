...
int isQueensSolvable(char queensArr[][SIZE], int dimension, int queensCounter, int x, int y, char zonesTaken[]) {
    ...
    printf("queens counter: %d", queensCounter);
    printf("X = %d, Y = %d\n", x, y);
    for (int i = 0; i < dimension; i++) {
        for (int j = 0; j < dimension; j++) {
            printf("%c ", queensArr[i][j]);
    ...
    for (int i = 0; i < dimension; i++) {
        for (int j = 0; j < dimension; j++) {
            ...
            char zone = queensArr[j][i];
            setZone(zonesTaken, dimension, zone);
            setQueen(queensArr, j, i);

            isQueensSolvable(queensArr, dimension, queensCounter + 1, j, i, zonesTaken);
...
void unsetZone(char zonesTaken[], int queensCounter, char zone) {
    zonesTaken[queensCounter + 1] = zone;
}
