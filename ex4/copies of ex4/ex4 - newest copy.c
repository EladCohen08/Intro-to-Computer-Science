...
int isBoardValid(char queensArr[][SIZE], int dimension, int x, int y, char zonesTaken[]);
...
int isQueensSolvable(char queensArr[][SIZE], int dimension, int queensCounter, int x, int y, char zonesTaken[]) {
    ...
    for (int i = 0; i < dimension; i++) {
        for (int j = 0; j < dimension; j++) {
            ...
            if (!isBoardValid(queensArr, dimension, j, i, zonesTaken)) {
                continue; // Skip invalid positions
            }

            // Set the queen, and the zone
            // Make it work for lower letters too!!
            char zone = queensArr[i][j];
            setZone(zonesTaken, queensCounter, zone);
            setQueen(queensArr, j, i);

            if (isQueensSolvable(queensArr, dimension, queensCounter + 1, j, i, zonesTaken)) {
                return 1;
            }

            // Backtrack if the recursive call fails
            unsetZone(zonesTaken, queensCounter, zone);
            unsetQueen(queensArr, j, i, zone);
        }
    }
    return 0;
}
...
int isBoardValid(char queensArr[][SIZE], int dimension, int x, int y, char zonesTaken[]) {
    ...
    // Return 0 if there is a queen in the same zone
    for (int i = 0; i < dimension; i++) {
...
void setZone(char zonesTaken[], int queensCounter, char zone) {
    ...
    printf("Set, ZONES TAKEN:\n");
    for (int i = 0; i <= queensCounter; i++) {
        printf("%c ", zonesTaken[i]);
    }
    printf("\n");
}
