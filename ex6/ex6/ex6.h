...
RESAULT removeFromBinTree(PokemonNode **rootPtr, int id);

// deletes a pokemon from a tree
...
// Deletes an entire tree post order
...
// Returns the root with the highest value
PokemonNode *findMaxBinTree(PokemonNode *root);

// Prints the tree height - maximal distance from the root
...
// Merges a single level of a tree to a different tree
...
// Finds the size of a tree given its root
